...
async function handleSignOut() {
  ...
  state.view = 'dashboard'; state.modal = null; state.dashboardMode = 'borrowed'
}
  ...
  if (addForm) {
    addForm.querySelectorAll('input, select').forEach((el) => {
      el.addEventListener('input', () => {
        ...
        // If the user is editing borrowed_from manually, clear the lenderId link
        // (so they don't accidentally keep an unrelated lender attached).
        if (addState.lenderId && addForm.borrowed_from?.value !== state.profiles[addState.lenderId]?.name) {
          addState.lenderId = null
          // Re-render to update the linked-pill UI
          // (focus stays on the input because we re-render after)
        }
